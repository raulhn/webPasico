...
async function registrarComponenteCarrusel(req, res) {
  ...
  const esAdministrador = await gestionUsuarios.esAdministrador(
    req.session.nombre
  );
  if (esAdministrador) {
    try {
      if (tipoAsociacion === constantes.TIPO_ASOCIACION_PAGINA) {
        await componente.registrarComponenteCarusel(
          id,
          tipoAsociacion,
        ...
        await componente.registrarComponenteCaruselOrden(
          id,
          tipoAsociacion,
          elementosSimultaneos,
          nOrden
...
async function obtenerComponenteCarrusel(req, res) {
  ...
  try {
    const resultadoCarrusel =
      await componenteCarrusel.obtenerComponenteCarusel(idComponente);
    const elementosCarrusel =
      await componenteCarrusel.obtenerElementosCarusel(idComponente);
    return res
      .status(200)
      .send({
        error: false,
        componenteCarrusel: resultadoCarrusel,
        elementosCarrusel,
      });
  } catch (error) {
    return res
      ...
      .send({
        error: true,
        message: "Error al obtener el componente carrusel",
      });
  }
}
...
async function addImagenCarrusel(req, res) {
  ...
  try {
    ...
    if (esAdministrador) {
      await componenteCarrusel.addElementoCarusel(
        idComponente,
        titulo,
        fichero
      );
      return res
        .status(200)
        .send({ error: false, message: "Se ha registrado el nuevo elemento" });
    } else {
      return res
        .status(400)
        .send({
...
async function actualizarElementosSimultaneos(req, res) {
  ...
  try {
    ...
    if (esAdministrador) {
      await componenteCarrusel.actualizaElementosSimultaneos(
        idComponente,
        numElementos
      );
      return res
  ...
  servletComun.comprobacionesLogin(req, res, async () => {
    ...
    if (tipoComponente === constantes.TIPO_COMPONENTE_TEXTO) {
      if (tipoAsociacion === constantes.TIPO_ASOCIACION_PAGINA) {
        ...
        await componente.registrarComponenteTextoOrden(
          id,
          tipoAsociacion,
          nOrden
        );
      ...
      if (tipoAsociacion === constantes.TIPO_ASOCIACION_PAGINA) {
        ...
        const nOrden = req.body.nOrden;
        await componente.registrarComponenteImagenOrden(
          id,
          titulo,
          tipoAsociacion,
          nOrden
      ...
      if (tipoAsociacion === constantes.TIPO_ASOCIACION_PAGINA) {
        ...
        const nOrden = req.body.nOrden;
        await componente.registrarComponenteVideoOrden(
          id,
          url,
          tipoAsociacion,
          nOrden
      ...
      if (tipoAsociacion === constantes.TIPO_ASOCIACION_PAGINA) {
        await componente.registrarComponenteGaleria(
          id,
          titulo,
          descripcion,
          tipoAsociacion
        ...
        const nOrden = req.body.nOrden;
        await componente.registrarComponenteGaleriaOrden(
          id,
          titulo,
          descripcion,
          tipoAsociacion,
      ...
      if (tipoAsociacion === constantes.TIPO_ASOCIACION_PAGINA) {
        await componenteComponentes.insertarComponenteComponentes(
          id,
          nColumnas,
          tipoAsociacion
        );
...
function obtenerTipoComponente(req, res) {
  ...
  componente.tipoComponente(idComponente).then(function (tipo) {
    return res
      .status(200)
...
function obtenerComponenteTexto(req, res) {
  ...
  try {
    componente.obtieneComponenteTexto(idComponente).then(function (resultado) {
      return res
        .status(200)
        .send({
          error: false,
...
function obtenerParametro(req, res) {
  ...
  try {
    parametro(identificador).then(function (resultado) {
...
function obtenerNumeroComponente(req, res) {
  ...
  componente.obtieneNumeroComponente(idPagina).then((numero) => {
    return res.status(200).send({ error: false, numero });
  });
}
...
function obtieneOrden(req, res) {
  ...
  componente.obtieneOrden(idPagina, idComponente).then((orden) => {
    return res.status(200).send({ error: false, orden });
  });
}

function obtenerComponentes(req, res) {
  componente
    .obtieneComponentes(req.params.id_pagina)
...
function incrementarOrden(req, res) {
  servletComun.comprobacionesLogin(req, res, async () => {
    ...
    componente
      .incrementaOrden(idPagina, idComponente)
      .then(() => {
        return res
          .status(200)
...
function decrementarOrden(req, res) {
  servletComun.comprobacionesLogin(req, res, () => {
    ...
    componente
      .decrementaOrden(idPagina, idComponente)
      .then(() => {
        return res
          .status(200)
          .send({ error: false, message: "Componente modificado" });
...
function eliminarComponente(req, res) {
  servletComun.comprobacionesLogin(req, res, () => {
    ...
    componente.tipoComponente(idComponente).then(async (tipo) => {
      if (tipo === constantes.TIPO_COMPONENTE_TEXTO) {
        console.log("Eliminar componente texto");
        componente
          .eliminarComponenteTexto(idPagina, idComponente, tipoAsociacion)
          .then(() => {
      ...
      if (tipo === constantes.TIPO_COMPONENTE_IMAGEN) {
        ...
        componente
          .eliminarComponenteImagen(idPagina, idComponente, tipoAsociacion)
          .then(() => {
            console.log("Eliminado");
            return res
              .status(200)
      ...
      if (tipo === constantes.TIPO_COMPONENTE_COMPONENTES) {
        ...
        await componenteComponentes.eliminarComponenteComponentes(
          idPagina,
          idComponente
        );
      ...
      if (tipo === constantes.TIPO_COMPONENTE_VIDEO) {
        ...
        componente
          .eliminarComponenteVideo(idPagina, idComponente, tipoAsociacion)
          .then(() => {
            console.log("Eliminado");
            return res
              .status(200)
      ...
      if (tipo === constantes.TIPO_COMPONENTE_GALERIA) {
        ...
        componente
          .eliminarComponenteGaleria(idPagina, idComponente, tipoAsociacion)
          .then(() => {
            console.log("Eliminado");
            return res
              .status(200)
      ...
      if (tipo === constantes.TIPO_COMPONENTE_PAGINAS) {
        ...
        componente
          .eliminarComponentePaginas(idPagina, idComponente, tipoAsociacion)
          .then(() => {
            console.log("Eliminado");
            return res
              .status(200)
      ...
      if (tipo === constantes.TIPO_COMPONENTE_CARUSEL) {
        ...
        try {
          await componente.eliminarComponenteCarusel(
            idPagina,
            idComponente,
            tipoAsociacion
          );
          ...
          return res
            .status(400)
            .send({ error: true, message: "Error al eliminar el componente" });
        }
      }
      if (tipo === constantes.TIPO_COMPONENTE_BLOG) {
        console.log("Eliminar componente Blog");
        try {
          await componenteBlog.eliminarComponenteBlog(
            idPagina,
            idComponente,
            tipoAsociacion
          );
