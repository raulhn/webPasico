...
function login (req, res) {
  ...
  gestionUsuarios.login(usuario, password).then(
    function (bResultado) {
      if (bResultado) {
...
function registrar (req, res) {
  ...
  conexion.dbConn.query('insert into ' + constantes.ESQUEMA_BD + '.usuario(usuario, pass) values(' + conexion.dbConn.escape(usuario) + ', ' + conexion.dbConn.escape(password) + ')',
