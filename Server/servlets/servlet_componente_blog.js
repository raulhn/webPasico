...
async function registrarComponenteBlog (req, res) {
  ...
  try {
    ...
    if (bAdministrador) {
      ...
      await componente.registrar_componente_comun(constantes.TIPO_COMPONENTE_BLOG, id, tipoAsociacion, nOrden)
...
async function addElementoBlog (req, res) {
  ...
  try {
    const bAdministrador = await usuario.esAdministrador(req.session.nombre)
    if (bAdministrador) {
      await componenteBlog.add_componente_blog(idComponente, titulo, fecha, fichero, descripcion)
...
async function obtenerComponenteBlog (req, res) {
  ...
  try {
    const resultado = await componenteBlog.obtener_componente_blog(idComponente)
...
async function eliminarElementoBlog (req, res) {
  ...
  if (bEsAdministrador) {
    ...
    try {
      await componenteBlog.eliminar_elemento_blog(idComponente, idImagen, idMenu)
      return res.status(200).send({ error: false, message: 'Elemento eliminado' })
    } catch (e) {
      console.log('Error al eliminar el elemento')
      return res.status(400).send({ error: true, message: 'Error al eliminar el elemento', info: e })
    }
  } else {
    return res.status(400).send({ error: true, message: 'Error al eliminar el elemento', info: 'Usuario no autorizado' })
  }
}
