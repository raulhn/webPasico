...
async function registrar_componente_blog(req, res)
{
    ...
    try{
        bAdministrador = await usuario.esAdministrador(req.session.nombre);
        if (bAdministrador)
        {
...
{
    bEsAdministrador = await usuario.esAdministrador(req.session.nombre);
    if (bEsAdministrador)
    {
        let id_componente = req.body.id_componente;
        ...
        try{
            await componente_blog.eliminar_elemento_blog(id_componente, id_imagen, id_menu)
