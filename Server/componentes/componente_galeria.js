  ...
  return new Promise(
    (resolve, reject) => {
      conexion.dbConn.beginTransaction(
        () => {
          imagen.subirImagen(titulo, fichero).then(
            (idImagen) => {
              ...
              conexion.dbConn.query('insert into ' + constantes.ESQUEMA_BD + '.galeria_imagen(nid_componente, nid_imagen) values(' +
                            conexion.dbConn.escape(idComponente) + ', ' + conexion.dbConn.escape(idImagen) + ')',

              (error, results, field) => {
                if (error) { console.log(error); conexion.dbConn.rollback(); reject(new Error('Error al añadir una imagen a la galeria')) } else {
            ...
            () => { conexion.dbConn.rollback(); reject(new Error('Error al añadir una imagen a la galeria')) }
          )
        }
      )
    }
    ...
    (resolve, reject) => {
      conexion.dbConn.beginTransaction(
        () => {
          ...
          (error, results, field) => {
            if (error) { console.log(error); conexion.dbConn.rollback(); reject(new Error('Error al eliminar la imagen de la galería')) } else {
...
function obtieneImagenesGaleria (idComponente) {
  return new Promise(
    (resolve, reject) => {
      conexion.dbConn.query('select nid_imagen from ' + constantes.ESQUEMA_BD + '.galeria_imagen where nid_componente = ' + conexion.dbConn.escape(idComponente),
        (error, results, field) => {
          if (error) { console.log(error); reject(new Error('Error la obtener las imagenes de la galeria')) } else {
            resolve(results)
          }
        }
