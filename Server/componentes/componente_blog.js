    ...
    (resolve, reject) => {
      conexion.dbConn.beginTransaction(
        async () => {
          ...
          const idPagina = await menu.registrarMenuId(titulo, idPadre, constantes.TIPO_PAGINA_GENERAL, '')
          ...
          conexion.dbConn.query(
            'insert into ' + constantes.ESQUEMA_BD + '.componente_blog(nid_componente, titulo, fecha, nid_imagen, nid_menu, descripcion) values(' +
                            conexion.dbConn.escape(idComponente) + ', ' + conexion.dbConn.escape(titulo) + ', ' +
                            'str_to_date(' + conexion.dbConn.escape(fecha) + ', "%Y-%m-%d")' + ', ' + conexion.dbConn.escape(idImagen) + ', ' +
                            conexion.dbConn.escape(idPagina) + ', ' + conexion.dbConn.escape(descripcion) + ')',
  ...
  return new Promise(
    (resolve, reject) => {
      conexion.dbConn.query('select nid_componente, titulo, fecha, nid_imagen, nid_menu, descripcion from ' + constantes.ESQUEMA_BD + '.componente_blog where nid_componente = ' +
                conexion.dbConn.escape(idComponente) + ' order by fecha desc',
      (error, results, fields) => {
        if (error) {
          console.log(error)
      ...
      conexion.dbConn.beginTransaction(
        async () => {
          conexion.dbConn.query('delete from ' + constantes.ESQUEMA_BD + '.componente_blog where nid_componente = ' +
                            ...
                            ' and nid_menu = ' + conexion.dbConn.escape(idMenu),
...
function numeroElementos (idComponente) {
  return new Promise(
    (resolve, reject) => {
      ...
      (error, results, fields) => {
        if (error) { console.log(error); resolve(0) } else { resolve(results[0].num) }
      }
      )
    }
  )
      ...
      conexion.dbConn.beginTransaction(
        async () => {
          ...
          if (cantidadElementos > 0) {
            try {
              await componente.eliminarComponenteComun(idComponente, idPagina, tipoAsociacion)
              resolve()
            } catch (error) {
              console.log(error)
              reject(error)
