      ...
      conexion.dbConn.beginTransaction(
        () => {
          conexion.dbConn.query('insert into ' + constantes.ESQUEMA_BD +
                            ...
                            ', instrumento2, familia_instrumento2, instrumento3, familia_instrumento3) values(' + conexion.dbConn.escape(nombre) + ', ' + conexion.dbConn.escape(primerApellido) + ', ' +
                            conexion.dbConn.escape(segundoApellido) + ', ' + conexion.dbConn.escape(dni) +
                            ', str_to_date(nullif(' + conexion.dbConn.escape(fechaNacimiento) + ', \'\') , \'%Y-%m-%d\'), ' + conexion.dbConn.escape(nombrePadre) +
                            ', ' + conexion.dbConn.escape(primerApellidoPadre) + ', ' + conexion.dbConn.escape(segundoApellidoPadre) + ', ' +
                            conexion.dbConn.escape(dniPadre) + ', ' + conexion.dbConn.escape(correoElectronico) + ', ' + conexion.dbConn.escape(telefono) +
                            ...
                            ', ' + conexion.dbConn.escape(codigoPostal) + ', ' + conexion.dbConn.escape(instrumento) + ', ' +
                            'nullif(' + conexion.dbConn.escape(familiaInstrumento) + ',\'\'), ' + conexion.dbConn.escape(sucursal) +
                            ',' + conexion.dbConn.escape(curso) + ', ' + conexion.dbConn.escape(horario) + ',' + conexion.dbConn.escape(tipoInscripcion) +
                             ', ' + conexion.dbConn.escape(instrumento2) + ', ' + 'nullif(' + conexion.dbConn.escape(familiaInstrumento2) + ',\'\'), ' +
                              conexion.dbConn.escape(instrumento3) + ', ' +
                            'nullif(' + conexion.dbConn.escape(familiaInstrumento3) + ',\'\') ' + ')',
          (error, results, fields) => {
            if (error) { console.log(error); conexion.dbConn.rollback(); reject(new Error('Error al registrar la preinscripción')) } else { conexion.dbConn.commit(); resolve() }
          }
          )
        }
      )
...
function obtenerPreinscripciones () {
  return new Promise(
    (resolve, reject) => {
      ...
      (error, results, fields) => {
        if (error) { console.log(error); reject(new Error('Error al obtener las preinscripicones')) } else { resolve(results) }
      })
    }
  )
}
...
function obtenerPreinscripcionesDetalle (nidPreinscripcion) {
  return new Promise(
    (resolve, reject) => {
      ...
      (error, results, fields) => {
        if (error) { console.log(error); reject(new Error('Error al obtener el detalle de la preinscripcion')) } else { resolve(results) }
      }
      )
    }
  )
