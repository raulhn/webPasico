...
import { generarIdempotencyKeyImpresion } from '../logica/utilidades.js';
...
export class FormularioPreinscripcionTorrePachecoComponent implements OnInit {
  ...
  constructor(
    ...
    private servicioPreinscripcion: ServicioPreinscripcionService,
  ...
  obtener_curso() {
    if (
      this.tipo_inscripcion == '1' &&
      this.calculo_edad() > 7 &&
      this.calculo_edad() < 18
    )
      return this.CURSO_PRIMERO;
    else if (
      this.tipo_inscripcion == '2' &&
      this.calculo_edad() > 7 &&
      this.calculo_edad() < 18
    )
      return this.curso_seleccionado;
    else if (this.calculo_edad() > 18) return this.CURSO_ADULTO;
    else if (this.calculo_edad() == 7) return this.CURSO_PREPARATORIO;
    else if (this.calculo_edad() < 7) return this.CURSO_INICIACION;
    ...
    next: (respuesta: any) => {
      ...
      if (this.calculo_edad() >= 7) {
        let data = {
          ...
          primer_apellido: this.primer_apellido_alumno,
          segundo_apellido: this.segundo_apellido_alumno,
          fecha_nacimiento: this.fecha_nacimiento,
          dni: this.dni_alumno,
          nombre_padre: this.nombre_padre,
          primer_apellido_padre: this.primer_apellido_padre,
          segundo_apellido_padre: this.segundo_apellido_padre,
          dni_padre: this.dni_padre,
          ...
          instrumento2: this.instrumentos_seleccionados[1].instrumento,
          familia_instrumento2:
            this.instrumentos_seleccionados[1].familia_instrumento,
        ...
        this.servicioPreinscripcion
          .registrar_preinscripcion(data)
          .subscribe(this.realiza_registro);
      } else {
        let data = {
          token: this.token,
          nombre: this.nombre_alumno,
  ...
  valida_formulario() {
    if (this.fecha_nacimiento == '') {
      ...
      this.lanza_error(
        'El alumno tiene que tener al menos 3 años para poder matricularse',
      );
      return false;
    }
    ...
    if (this.calculo_edad() < 18) {
      if (this.nombre_padre == '') {
        this.lanza_error(
          'Es necesario instroducir un nombre para el Padre / Madre / Tutor',
        );
        return false;
      } else if (this.primer_apellido_padre == '') {
        this.lanza_error(
          'Es necesario instroducir el primer apellido para el Padre / Madre / Tutor',
        );
        return false;
      } else if (this.dni_padre == '') {
        this.lanza_error(
          'Es necesario introducir el DNI para el Padre / Madre / Tutor',
        );
        return false;
      }
    }
    ...
    if (this.nombre_alumno.length > 100) {
      this.lanza_error(
        'El nombre del alumno no puede tener más de 100 caracteres',
      );
      return false;
    } else if (this.primer_apellido_alumno.length > 100) {
      this.lanza_error(
        'El primer apellido del alumno no puede tener más de 100 caracteres',
      );
      return false;
    } else if (this.segundo_apellido_alumno.length > 100) {
      this.lanza_error(
        'El segundo apellido del alumno no puede tener más de 100 caracteres',
      );
      return false;
    } else if (this.nombre_padre.length > 100) {
      this.lanza_error(
        'El nombre del Padre / Madre / Tutor no puede tener más de 100 caracteres',
      );
      return false;
    } else if (this.primer_apellido_padre.length > 100) {
      this.lanza_error(
        'El primer apellido del Padre / Madre / Tutor no puede tener más de 100 caracteres',
      );
      return false;
    } else if (this.segundo_apellido_padre.length > 100) {
      this.lanza_error(
        'El segundo apellido del Padre / Madre / Tutor no puede tener más de 100 caracteres',
      );
      return false;
      ...
      this.lanza_error(
        'El DNI del Padre / Madre / Tutor supera el maximo de caracteres',
      );
      return false;
      ...
      this.lanza_error(
        'El número de la dirección supera el máximo de caracteres',
      );
      return false;
  ...
  add_instrumento(num_instrumento: number) {
    Swal.fire({
      ...
      if (results.isConfirmed) {
        this.instrumentos_seleccionados[num_instrumento].familia_instrumento =
          this.familia_instrumento;
        this.instrumentos_seleccionados[num_instrumento].instrumento =
          this.instrumento;
      }
    });
  }
