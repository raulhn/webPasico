...
async function insertarPartitura(req, res) {
    try {
        ...
        let rolDirector = await servletComun.comprobarRol(req, res, rolesPermitidos);
        if (!rolDirector) {
          res.status(403).send({
            error: true,
        ...
        const result = await gestorPartituras.insertarPartitura(titulo, autor, categoria);
        res.status(200).json({ message: "Partitura insertada correctamente", result });
        }
...
async function actualizarPartitura(req, res) {
    try {
        ...
        const { nid_partitura, titulo, autor, categoria } = req.body;
        const result = await gestorPartituras.actualizarPartitura(nid_partitura, titulo, autor, categoria);
        res.status(200).json({ message: "Partitura actualizada correctamente", result });}
    } catch (error) {
