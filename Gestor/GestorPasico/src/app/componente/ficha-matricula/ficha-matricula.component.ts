...
export class FichaMatriculaComponent implements OnInit {
  ...
  click_matricula_asignatura(asignatura_seleccionada: any) {
    this.matricula_asignatura_seleccionada =
      asignatura_seleccionada['nid_matricula_asignatura'];
  ...
  recuperar_matricula = {
    next: (respuesta: any) => {
      ...
      this.comentario_precio =
        respuesta['matricula']['comentario_precio_manual'];
    },
  };
  ...
  obtener_mensualidad_matricula = {
    next: (respuesta: any) => {
      ...
      if (
        this.mensualidad_matricula === undefined ||
        this.mensualidad_matricula === null
      ) {
  ...
  ngOnInit(): void {
    this.matriculaService
      .obtener_matricula(this.nid_matricula)
      .subscribe(this.recuperar_matricula);
    this.matriculaService
      .obtener_asignaturas_matriculas(this.nid_matricula)
      .subscribe(this.obtener_asignaturas);
    this.evaluacionesService
      .obtener_evaluacion_matricula_asignatura(this.nid_matricula)
      .subscribe(this.recuperar_evaluaciones);
    this.remesaService
      .obtener_precio_mensualidad(this.nid_matricula)
      .subscribe(this.obtener_mensualidad_matricula);
    this.evaluacionesService
      .obtener_trimestres()
  ...
  guardar() {
    this.matriculaService
      .registrar_precio_manual(
        this.nid_matricula,
        this.precio_manual,
        this.comentario_precio,
      )
      .subscribe(this.registrar_precio);
  }
  ...
  generar_boletin() {
    Swal.fire({
      ...
      if (results.isConfirmed) {
        this.evaluacionesService
          .generar_boletin(this.nid_matricula, this.trimestre_seleccionado)
      ...
      if (results.isConfirmed) {
        if (this.fecha_alta_seleccionada == '') {
          ...
          this.matriculaService
            .actualizar_fecha_alta(
              this.matricula_asignatura_seleccionada,
              this.fecha_alta_seleccionada,
            )
            .subscribe(this.peticion_actualizar_fecha);
        }
      }
    });
  }
