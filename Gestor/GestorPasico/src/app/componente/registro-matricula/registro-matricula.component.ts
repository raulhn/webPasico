...
export class RegistroMatriculaComponent implements OnInit {
  ...
  obtener_cursos = {
    next: (respuesta: any) => {
      this.cursos = respuesta.cursos.map((elemento: any) => {
      ...
      this.matriculasServices
        .obtener_alumnos_asignaturas(
          this.cursos[0]['clave_curso'],
          this.nid_asignatura,
          this.activo,
        )
        .subscribe(this.obtener_alumnos);
  ...
  obtener_personas = {
    next: (respuesta: any) => {
      this.lista_personas = respuesta.personas.map((elemento: any) => {
        return { etiqueta: elemento.etiqueta, clave_persona: elemento.nid };
      });
      this.alumno = '';
    },
  };
  ...
  obtener_profesores = {
    next: (respuesta: any) => {
      this.lista_profesores = respuesta.profesores.map((elemento: any) => {
        return {
          etiqueta_profesor: elemento.etiqueta,
          clave_profesor: elemento.nid_persona,
        };
      });
  ...
  ngOnInit(): void {
    ...
    this.asignaturasServices
      .obtener_asignatura(this.nid_asignatura)
      .subscribe(this.obtener_asignatura);
  ...
  onChangeCurso(evento: any) {
    this.matriculasServices
      .obtener_alumnos_asignaturas(this.curso, this.nid_asignatura, this.activo)
      .subscribe(this.refrescar_alumnos);
  }
    ...
    Swal.fire({
      ...
      if (results.isConfirmed) {
        this.matriculasServices
          ...
          .subscribe(this.registrar_alumno);
      }
    });
  }
