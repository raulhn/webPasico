...
export class PadresPersonaComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.personaService
      .obtener_padre(this.nid_persona)
      .subscribe(this.obtener_padre);
  }
  ...
  guardar() {
    if (this.padre === undefined) {
      this.personaService
        .registrar_padre(this.nid_persona, '')
        .subscribe(this.registrar_padre);
    } else {
      this.personaService
        .registrar_padre(this.nid_persona, this.padre)
        .subscribe(this.registrar_padre);
    }
  }
