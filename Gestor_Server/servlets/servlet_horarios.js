    ...
    async () =>
        {
            ...
            await horarios.crear_horario(dia, hora_inicio, minutos_inicio, hora_fin, minutos_fin, nid_asignatura, nid_profesor, duracion_clase);

            res.status(200).send({error:false, meessage: 'Se ha registrado el horario'})
        }
  )  
}
...
function registrar_horario_clase(req, res)
{
    comun.comprobaciones(req, res,
        async () => 
        {
            ...
            await horarios.registrar_horario_clase(hora_inicio, minutos_inicio, duracion_clase, 0, nid_horario);

            res.status(200).send({error:false, message: 'Se ha registrado el horario'})
        }
    )
}
        ...
        async () =>
            {
                ...
                res.status(200).send({error:false, message: 'Se ha eliminado el horario'})
            }
    )
}
        ...
        async () =>
            {
                ...
                await horarios.asignar_horario_clase(nid_horario_clase, nid_matricula_asignatura);

                res.status(200).send({error: false, message: 'Se ha asigando el horario'});
            }
    )
}
...
function obtener_horario_profesor(req, res)
{
    comun.comprobaciones(req, res,
        async () =>
        {
            ...
            let horarios_clase_recuperados = await horarios.obtener_horarios_profesor(nid_profesor);
...
function obtener_alumnos_horario_clase(req, res)
{
    comun.comprobaciones(req, res,
        async () =>
        {
            ...
            let alumnos_recuperados = await horarios.obtener_alumnos_horario_clase(nid_horario_clase);
...
function obtener_alumnos_sin_asignar(req, res)
{
    comun.comprobaciones(req, res,
        async() =>
        {
            ...
            let alumnos_recuperados = await horarios.obtener_alumnos_sin_asignar(nid_horario_clase);
