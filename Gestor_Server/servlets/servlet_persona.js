...
async function obtener_personas_tipo(req, res) {
  comun.comprobaciones(req, res, async () => {
    ...
    if (tipo == "1") {
      ...
      res
        .status(401)
        .send({ error: true, message: "No se ha encontrado el tipo" });
    }
  });
}
  ...
  comun.comprobaciones(req, res, async () => {
    try {
      ...
      if (bResultado) {
        ...
        await persona.actualizar_sucio(nid, "S");
        res.status(200).send({
          error: false,
      ...
      console.error("Error en la función actualizar_persona:", error);
      res.status(500).send({
        error: true,
...
async function registrar_padre(req, res) {
  comun.comprobaciones(req, res, async () => {
    ...
    await persona.actualizar_sucio(nid_persona, "S");
...
async function obtener_forma_pagos_persona(req, res) {
  comun.comprobaciones(req, res, async () => {
    let nid_titular = req.params.nid_titular;
    let formas_pago = await persona.obtener_formas_pago_persona(nid_titular);

    res.status(200).send({ error: false, formas_pago: formas_pago });
  });
}

async function obtener_forma_pago_nid(req, res) {
  comun.comprobaciones(req, res, async () => {
    let nid_forma_pago = req.params.nid_forma_pago;
    let forma_pago = await persona.obtener_forma_pago_nid(nid_forma_pago);

    res.status(200).send({ error: false, forma_pago: forma_pago });
  });
}
...
async function asociar_forma_pago(req, res) {
  comun.comprobaciones(req, res, async () => {
    ...
    res
      .status(200)
      .send({ error: false, message: "Se ha asociado la forma de pago" });
  });
}
...
async function obtener_persona_apellidos(req, res) {
  comun.comprobaciones(req, res, async () => {
    ...
    let resultados = await persona.obtener_persona_apellidos(
      primer_apellido,
      segundo_apellido
    );
...
async function actualizar_forma_pago(req, res) {
  comun.comprobaciones(req, res, async () => {
    let nid_forma_pago = req.body.nid_forma_pago;
    let activo = req.body.activo;

    await persona.actualizar_forma_pago(nid_forma_pago, activo);

    res.status(200).send({ error: false, message: "Registro actualizado" });
  });
}
...
module.exports.obtener_forma_pagos_persona = obtener_forma_pagos_persona;
