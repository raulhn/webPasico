...
function registrar_matricula(req, res) {
  comun.comprobaciones(req, res, async () => {
    ...
    let bExisteMatricula = await matricula.existe_matricula(
      nid_persona,
      nid_curso
    );
    ...
    let bExisteAsignatura = await asignatura.existe_asignatura(nid_asignatura);

    if (bExisteAsignatura) {
      await matricula.add_asignatura(
        nid_matricula,
        nid_asignatura,
        nid_profesor
      );
...
function obtener_matriculas(req, res) {
  comun.comprobaciones(req, res, async () => {
    let resultados = await matricula.obtener_matriculas();
...
function obtener_cursos_profesor(req, res) {
  comun.comprobaciones(req, res, async () => {
    ...
    let resultados = await matricula.obtener_cursos_profesor(nid_profesor);
...
function obtener_matriculas_alumno(req, res) {
  comun.comprobaciones(req, res, async () => {
    ...
    let resultados = await matricula.obtener_matriculas_alumno(nid_alumno);
    res.status(200).send({ error: false, matriculas: resultados });
  });
}
...
function actualizar_fecha_alta_matricula_asignatura(req, res) {
  comun.comprobaciones(req, res, async () => {
    ...
    await matricula.actualizar_fecha_alta_matricula_asignatura(
      nid_matricula_asignatura,
      fecha_alta
    );
...
function actualizar_fecha_baja_matricula_asignatura(req, res) {
  comun.comprobaciones(req, res, async () => {
    ...
    await matricula.actualizar_fecha_baja_matricula_asignatura(
      nid_matricula_asignatura,
      fecha_baja
    );
