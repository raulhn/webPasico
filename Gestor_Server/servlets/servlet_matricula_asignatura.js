...
function obtener_matriculas_asignaturas_alumno(req, res) {
    ...
    comun.comprobaciones_profesor(req, res, async() => {
        ...
        if(bEsProfesor)
        {
            let resultados = await matricula.obtener_asignaturas_matricula(nid_matricula);
            res.status(200).send({error: false, mensaje: 'Matriculas de asignaturas obtenidas correctamente', matriculas_asignaturas: resultados, 
                                nombre_alumno: v_matricula.nombre_alumno});
        }
        else
        {
            res.status(403).send({error: true, mensaje: 'El profesor no tiene acceso a las matriculas de este alumno'});
        }
    }
    )
}
