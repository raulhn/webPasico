...
function registrar_socio(req, res)
{
    comun.comprobaciones(req, res,
        async () =>
        {
            ...
            res.status(200).send({error: false, message: 'Se ha dado de alta al socio'});
        }
    )
}
...
function actualizar_socio(req, res)
{
    comun.comprobaciones(req, res,
        async () =>
        {
            ...
            await socios.actualizar_socio(nid_persona, num_socio, fecha_alta, fecha_baja);
            res.status(200).send({error: false, message: 'Se ha actualizado el socio'})
        }
    )
}
