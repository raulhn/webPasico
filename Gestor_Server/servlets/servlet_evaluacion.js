...
function registrar_evaluacion(req, res) {
  comun.comprobaciones(req, res, async () => {
    ...
    let nid_evaluacion = await evaluacion.registrar_evaluacion(
      nid_trimestre,
      nid_asignatura,
      nid_profesor
    );
    ...
    for (let i = 0; i < array_persona.length; i++) {
      ...
      await evaluacion.registrar_evaluacion_matricula(
        nid_evaluacion,
...
function obtener_evaluacion(req, res) {
  comun.comprobaciones(req, res, async () => {
    ...
    let bExiste_evaluacion = await evaluacion.existe_evaluacion(
      nid_trimestre,
      nid_asignatura,
      nid_profesor
    );
    ...
    if (bExiste_evaluacion) {
      let evaluacion_recuperada = await evaluacion.obtener_evaluacion(
        nid_trimestre,
        nid_asignatura,
        nid_profesor
      );

      let evaluaciones_matriculas =
        await evaluacion.obtener_evaluaciones_matricula(
          evaluacion_recuperada["nid_evaluacion"]
        );

      res.status(200).send({
        error: false,
        evaluacion: evaluacion_recuperada,
        evaluaciones_matriculas: evaluaciones_matriculas,
      });
    } else {
      res
        .status(200)
        .send({ error: false, evaluacion: null, evaluaciones_matriculas: [] });
    }
  });
}
...
function obtener_evaluacion_matricula_asginatura(req, res) {
  comun.comprobaciones(req, res, async () => {
    ...
    let resultado = await evaluacion.obtener_evaluacion_matricula_asginatura(
      nid_matricula
    );
  ...
  comun.comprobaciones_profesor(req, res, async () => {
    ...
    if (bEsProfesor) {
      ...
      res.status(403).send({
        error: true,
        message:
          "El profesor no tiene acceso a las evaluaciones de este alumno",
      });
    }
  });
}
...
function generar_boletin(req, res) {
  comun.comprobaciones(req, res, async () => {
    ...
    let fichero = await evaluacion.generar_boletin(
      nid_matricula,
      nid_trimestre
    );
...
module.exports.obtener_evaluacion_matricula_asignatura =
  obtener_evaluacion_matricula_asginatura;
module.exports.obtener_evaluacion_matricula_asignatura_profesor =
  obtener_evaluacion_matricula_asignatura_profesor;
