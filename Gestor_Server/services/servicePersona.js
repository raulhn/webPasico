...
function fetchWithTimeout(url, options, timeout = 5000) { // Timeout en milisegundos (5 segundos por defecto)
    return Promise.race([
        fetch(url, options),
        new Promise((_, reject) =>
            setTimeout(() => reject(new Error('Timeout: El servicio no respondió a tiempo')), timeout)
        )
    ]);
}
...
function obtener_persona(nid_persona) {
    return new Promise((resolve, reject) => {
        fetchWithTimeout(constantes.URL_SERVICIO_MOVIL + 'obtener_persona/' + nid_persona, {
            method: 'GET',
            headers: {
                'Content-Type': 'application/json',
...
function obtenerPersonasSucias()
{
    ...
    return new Promise((resolve, reject) => {
        fetchWithTimeout(constantes.URL_SERVICIO_MOVIL + 'obtener_personas_sucias', {
            method: 'GET',
            headers: {
                'Content-Type': 'application/json',
                'x-api-key': process.env.API_KEY_MOVIL,
