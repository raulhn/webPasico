...
function obtener_siguiente_id()
{
    return new Promise(
        (resolve, reject) =>
        {
            conexion.dbConn.query('select nvl(max(nid_imagen), \'0\') siguiente_id from ' + constantes.ESQUEMA_BD + '.imagenes',
                (error, results, fields) =>
                {
                    if(error){console.log(error); reject(error)}
                    else {resolve(Number(results[0]['siguiente_id']) + 1)}
                }
            )
        }
    )
         ...
         (resolve, reject) =>
         {
                ...
                async () =>
                {
                        try
                        {
                            ...
                            conexion.dbConn.query('insert into ' + constantes.ESQUEMA_BD + '.imagenes(nid_imagen, path, nombre) values(' +
                                    conexion.dbConn.escape(nid_imagen) + ', ' +
                                    conexion.dbConn.escape(constantes.RUTA_SUBIDAS) + ', ' + conexion.dbConn.escape(nombre_fichero)  + ')',
                                (error , results, fields) =>
                                {
...
function obtener_ruta(nid_imagen)
{
    return new Promise(
        (resolve, reject) =>
        {
            conexion.dbConn.query('select concat(path, nombre) ruta from ' + constantes.ESQUEMA_BD + '.imagenes where nid_imagen = ' +
                   conexion.dbConn.escape(nid_imagen),
                (error, results, fields) =>
                {
                    if(error) {console.log(error); reject(error);}
                    else if (results.length < 1) {reject('No se ha encontrado la imagen')}
                    else {resolve(results[0]['ruta'])}
                }
            )
        }
    )
...
function obtener_imagen(nid_imagen)
{
    return new Promise(
        async (resolve, reject) =>
        {
            try
            {
                ...
                {
                    fs.readFile(constantes.IMAGEN_NO_ENCONTRADA,
                    (error, data) =>
                    {
                        if(error) {console.log(error); reject(error)}
                        else {resolve(data)}
