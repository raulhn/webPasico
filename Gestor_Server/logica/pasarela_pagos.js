...
async function async_existe_usuario_pasarela_pago(
  nid_persona,
  resolve,
  reject
) {
  try {
    ...
    let v_nid_pasarela_pago = v_persona["nid_pasarela_pago"];
...
function existe_usuario_pasarela_pago(nid_persona) {
  return new Promise((resolve, reject) => {
    async_existe_usuario_pasarela_pago(nid_persona, resolve, reject);
  });
}

async function async_crear_usuario(nid_persona, resolve, reject) {
  try {
...
function crear_usuario(nid_persona) {
  return new Promise((resolve, reject) => {
    async_crear_usuario(nid_persona, resolve, reject);
  });
}

async function async_crear_metodo_pago_cuenta_bancaria(
  nid_forma_pago,
  resolve,
  reject
) {
  try {
    ...
    if (bExiste_persona) {
      ...
      let metodo_pago = await servicio_stripe.paymentMethods.create(data);
...
function crear_metodo_pago_cuenta_bancaria(nid_forma_pago) {
  return new Promise((resolve, reject) => {
    async_crear_metodo_pago_cuenta_bancaria(nid_forma_pago, resolve, reject);
  });
}

async function async_cobrar_pago(
  nid_forma_pago,
  descripcion,
  cantidad,
  p_ip_address,
...
async function async_cobrar_remesa(
  nid_remesa,
  p_ip_address,
  p_user_agent,
  resolve,
...
async function async_recuperar_pago(nid_pasarela_pago, resolve, reject) {
  try {
    let servicio_stripe = stripe(pagos.KEY);
    ...
    console.log("Error al recuperar el pago " + error);
    reject("Error al recuperar el pago");
  }
}

function recuperar_pago(nid_pasarela_pago) {
  return new Promise((resolve, reject) => {
    async_recuperar_pago(nid_pasarela_pago, resolve, reject);
  });
}

async function async_cobrar_lote(
  nid_lote,
  p_ip_address,
  p_user_agent,
  resolve,
...
function cobrar_lote(nid_lote, p_ip_address, p_user_agent) {
  return new Promise((resolve, reject) => {
    async_cobrar_lote(nid_lote, p_ip_address, p_user_agent, resolve, reject);
  });
}
...
module.exports.recuperar_pago = recuperar_pago;
