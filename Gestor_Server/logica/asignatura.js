            ...
            conexion.dbConn.beginTransaction(
                () =>
                {
                    conexion.dbConn.query("update " + constantes.ESQUEMA_BD + ".asignatura set descripcion = " + 
                        conexion.dbConn.escape(descripcion)  +
                        ", tipo_asignatura = " + conexion.dbConn.escape(tipo_asignatura) +
...
function obtener_asignatura(nid_asignatura)
{
    return new Promise(
        (resolve, reject) =>
        {
            conexion.dbConn.query("select nid, descripcion, tipo_asignatura as tipo_asignatura from " + constantes.ESQUEMA_BD + '.asignatura where nid = ' + conexion.dbConn.escape(nid_asignatura),
                (error, results, fields) =>
                {
                    if(error) {console.log(error); reject();}
...
function obtener_profesores_asignatura(nid_asignatura)
{
    return new Promise(
        (resolve, reject) =>
        {
            conexion.dbConn.query("select p.*, p.nid nid_persona, concat(ifnull(p.nif, \' \'), \' \',  p.nombre, \' \', ifnull(p.primer_apellido, '\ '\), \' \' , ifnull(p.segundo_apellido, '\ '\)) etiqueta, a.nid nid_asignatura, a.descripcion from " + constantes.ESQUEMA_BD + ".persona p, " + 
