...
async function obtener_siguiente_lote() {
  try {
    ...
    console.log("remesa.js - obtener_siguiente_lote -> " + error);
    throw new Error("Error al obtener el siguiente lote");
  }
}

async function registrar_remesa(
  v_persona,
  v_concepto,
  v_siguiente_lote,
  v_precio,
  ...
  try {
    ...
    console.log("remesa.js - registrar_remesa -> " + error);
    throw new Error("Error al registrar la remesa");
  }
}

async function registrar_linea_remesa(v_remesa, v_precio, v_concepto) {
  try {
    const sql =
      "insert into " +
      constantes.ESQUEMA_BD +
    ...
    console.log("remesa.js - registrar_linea_remesa -> " + error);
    throw new Error("Error al registrar la línea de remesa");
  }
}

async function registrar_descuento(nid_remesa, concepto) {
  try {
    const sql =
      "insert into " +
      constantes.ESQUEMA_BD +
    ...
    console.log("remesa.js - registrar_descuento -> " + error);
    throw new Error("Error al registrar el descuento de remesa");
  }
}

async function obtener_matriculas_activas(nid_socio) {
  try {
    const sql =
    ...
    console.log("remesa.js - obtener_matriculas_activas -> " + error);
    throw new Error("Error al obtener las matriculas activas");
  }
}

async function obtener_matriculas_activas_fecha(
  nid_socio,
  fecha_desde,
  fecha_hasta,
) {
  try {
    ...
    console.log("remesa.js - obtener_matriculas_activas_fecha -> " + error);
    throw new Error("Error al obtener las matriculas activas por fecha");
  }
}

async function comprueba_es_socio(nid_persona) {
  try {
    const sql =
      " select 'S' " +
      "from " +
    ...
    console.log("remesa.js - comprueba_es_socio -> " + error);
    throw new Error("Error al comprobar si es socio");
  }
}

async function obtener_nid_socio(nid_persona) {
  try {
    const sql =
    ...
    if (results.length > 0) {
      ...
      const results_socio = await gestor_base_datos.consulta(sql_socio);
      return results_socio[0]["nid_socio"];
    } else {
      console.log(
        "Error al obtener el nid_socio para la persona: " + nid_persona,
      );
      throw new Error(
        "Error al obtener el nid_socio para la persona: " + nid_persona,
      );
    ...
    console.log("remesa.js - obtener_nid_socio -> " + error);
    throw new Error(
      "Error al obtener el nid_socio para la persona: " + nid_persona,
    );
  }
...
async function eliminar_descuento_lote(v_lote) {
  try {
    const sql =
      "delete from " +
      constantes.ESQUEMA_BD +
    ...
    console.log("remesa.js - eliminar_descuento_lote -> " + error);
    throw new Error("Error al eliminar los descuentos de la remesa");
  }
}

async function eliminar_linea_remesa_lote(v_lote) {
  try {
    const sql =
      "delete from " +
      constantes.ESQUEMA_BD +
    ...
    console.log("remesa.js - eliminar_linea_remesa_lote -> " + error);
    throw new Error("Error al eliminar las líneas de remesa");
  }
}
...
async function obtener_remesas(fecha_desde, fecha_hasta) {
  try {
    const sql =
    ...
    console.log("remesa.js - obtener_remesas -> " + error);
    throw new Error("Error al obtener las remesas");
  }
}

async function obtener_remesa(lote) {
  try {
    const sql =
      "select r.nid_remesa, r.nid_forma_pago, r.nid_persona, r.concepto, r.lote, r.precio, r.estado, r.anotaciones, r.nid_cobro_pasarela_pago " +
    ...
    console.log("remesa.js - obtener_remesa -> " + error);
    throw new Error("Error al obtener la remesa");
  }
}

async function obtener_remesa_pendiente(lote) {
  try {
    const sql =
      "select r.nid_remesa, r.nid_forma_pago, r.nid_persona, r.concepto, r.lote, r.precio, r.estado, r.anotaciones, r.nid_cobro_pasarela_pago " +
    ...
    console.log("remesa.js - obtener_remesa_pendiente -> " + error);
    throw new Error("Error al obtener la remesa pendiente");
  }
}

async function obtener_remesa_estado(lote, estado) {
  try {
    const sql =
      "select r.nid_remesa, r.nid_forma_pago, r.nid_persona, r.concepto, r.lote, r.precio, r.estado, r.anotaciones, r.nid_cobro_pasarela_pago " +
      ", DATE_FORMAT(r.fecha, '%d/%m/%Y') fecha from " +
    ...
    console.log("remesa.js - obtener_remesa_estado -> " + error);
    throw new Error("Error al obtener la remesa por estado");
  }
}

async function obtener_lineas_remesa(nid_remesa) {
  try {
    const sql =
      "select * from " +
      constantes.ESQUEMA_BD +
    ...
    console.log("remesa.js - obtener_lineas_remesa -> " + error);
    throw new Error("Error al obtener las líneas de remesa");
  }
}

async function obtener_descuentos_remesa(nid_remesa) {
  try {
    const sql =
      "select * from " +
      constantes.ESQUEMA_BD +
    ...
    console.log("remesa.js - obtener_descuentos_remesa -> " + error);
    throw new Error("Error al obtener los descuentos de remesa");
  }
}
...
async function obtener_ultimo_lote() {
  try {
    const sql =
    ...
    const results = await gestor_base_datos.consulta(sql);
    return results[0]["ultimo_lote"];
  } catch (error) {
    console.log("remesa.js - obtener_ultimo_lote -> " + error);
    throw new Error("Error al obtener el último lote");
  }
}

async function obtener_remesa_nid(nid_remesa) {
  try {
    const sql =
      "select r.nid_remesa, r.nid_forma_pago, r.nid_persona, r.concepto, r.lote, r.precio, r.estado, r.anotaciones, r.nid_cobro_pasarela_pago " +
      ", DATE_FORMAT(r.fecha, '%d/%m/%Y') fecha from " +
    ...
    const results = await gestor_base_datos.consulta(sql);
    if (results.length == 0) {
      throw new Error("No se ha encontrado la remesa");
    } else {
      return results[0];
    }
  } catch (error) {
    console.log("remesa.js - obtener_remesa_nid -> " + error);
    throw new Error("Error al obtener la remesa por nid");
  }
}
...
async function actualizar_estado(nid_remesa, estado, anotaciones) {
  try {
    const sql =
      "update " +
      constantes.ESQUEMA_BD +
    ...
    console.log("remesa.js - actualizar_estado -> " + error);
    throw new Error("Error al actualizar el estado de la remesa");
  }
}
...
async function actualizar_id_cobro_pasarela_pago(
  nid_remesa,
  nid_cobro_pasarela,
) {
  try {
    ...
    console.log("remesa.js - actualizar_id_cobro_pasarela_pago -> " + error);
    throw new Error("Error al actualizar el cobro");
  }
}

async function actualizar_remesa(nid_remesa, precio, concepto, estado) {
  try {
    const sql =
      "update " +
      constantes.ESQUEMA_BD +
    ...
    console.log("remesa.js - actualizar_remesa -> " + error);
    throw new Error("Error al actualizar la remesa");
  }
}

async function actualizar_linea_remesa(nid_linea_remesa, concepto, precio) {
  try {
    const sql =
      "update " +
      constantes.ESQUEMA_BD +
    ...
    console.log("remesa.js - actualizar_linea_remesa -> " + error);
    throw new Error("Error al actualizar la línea de remesa");
  }
}

async function actualizar_remesa_descuento(nid_remesa_descuento, concepto) {
  try {
    const sql =
      "update " +
      constantes.ESQUEMA_BD +
    ...
    console.log("remesa.js - actualizar_remesa_descuento -> " + error);
    throw new Error("Error al actualizar el descuento de remesa");
  }
}
...
async function nueva_linea_remesa(nid_remesa, concepto, precio) {
  try {
    const sql =
      "insert into " +
      constantes.ESQUEMA_BD +
    ...
    throw new Error("Error al crear la línea de remesa");
  }
}

async function nuevo_descuento_remesa(nid_remesa, concepto) {
  try {
    const sql =
      "insert into " +
      constantes.ESQUEMA_BD +
    ...
    throw new Error("Error al crear el descuento de remesa");
  }
}

async function eliminar_linea_remesa(nid_linea_remesa) {
  try {
    const sql =
      "delete from " +
      constantes.ESQUEMA_BD +
    ...
    throw new Error("Error al eliminar la línea de remesa");
  }
}

async function eliminar_descuento_remesa(nid_descuento_remesa) {
  try {
    const sql =
      "delete from " +
      constantes.ESQUEMA_BD +
    ...
    throw new Error("Error al eliminar el descuento de remesa");
  }
}
