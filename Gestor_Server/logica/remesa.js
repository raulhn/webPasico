...
async function async_precio_matricula(
  nid_matricula,
  num_familiar,
  resolve,
  reject
  ...
  let asignaturas_precio =
    await gestor_matricula.obtener_asignaturas_matricula_activas(nid_matricula);
  ...
  if (
    ...
    for (let z = 0; z < asignaturas_precio.length; z++) {
      v_precio_persona = 0;
      let linea_remesa = new Object();

      let v_tipo_asignatura = asignaturas_precio[z]["tipo_asignatura"];
    ...
    if (!es_socio) {
      resumen_matricula.precio =
        parseFloat(resumen_matricula.precio) + parseFloat(SUMA_PRECIO_NO_SOCIO);
      descuentos.push(
    ...
    if (instrumento_banda && instrumento_cuerda && es_socio) {
      resumen_matricula.precio =
        parseFloat(resumen_matricula.precio) - parseFloat(REBAJA_VIENTO_CUERDA);
      descuentos.push(
        "-" +
...
async function precio_matricula(nid_matricula, num_familiar) {
  return new Promise((resolve, reject) => {
    async_precio_matricula(nid_matricula, num_familiar, resolve, reject);
  });
}

async function async_precio_matricula_fecha(
  nid_matricula,
  num_familiar,
  fecha_desde,
  fecha_hasta,
  ...
  try {
    ...
    if (
      ...
      linea_remesa = new Object();

      linea_remesa.precio = datos_matricula["precio_manual"];

      diferencia_dias = Math.round(
    ...
    console.log(error);
    reject("Error al recuperar el precio de la mensualidad");
  }
}

function precio_matricula_fecha(
  nid_matricula,
  num_familiar,
  fecha_desde,
  fecha_hasta
...
async function async_registrar_remesa_matriculas(v_concepto, resolve, reject) {
  try {
...
function registrar_remesa_matriculas(v_concepto) {
  return new Promise((resolve, reject) => {
    async_registrar_remesa_matriculas(v_concepto, resolve, reject);
  });
}

async function async_registrar_remesa_matriculas_fecha(
  v_concepto,
  fecha_desde,
  fecha_hasta,
  resolve,
  ...
  try {
    ...
    console.log(error);
    reject("Error al registrar la remesa");
  }
}

function registrar_remesa_matriculas_fecha(
  v_concepto,
  fecha_desde,
  fecha_hasta
) {
  return new Promise((resolve, reject) => {
    async_registrar_remesa_matriculas_fecha(
      v_concepto,
      fecha_desde,
      fecha_hasta,
      resolve,
...
async function async_registrar_remesa_persona(
  nid_matricula,
  v_concepto,
  lote,
  resolve,
...
function registrar_remesa_persona(nid_matricula, v_concepto, lote) {
  return new Promise((resolve, reject) => {
    async_registrar_remesa_persona(
      nid_matricula,
      v_concepto,
      lote,
      resolve,
...
async function async_registrar_remsa_persona_fecha(
  nid_matricula,
  v_concepto,
  lote,
  fecha_desde,
    ...
    if (bEs_socio) {
      ...
      if (v_personas_activas !== undefined) {
        if (v_personas_activas.length > 0) {
          ...
          reject("No se ha podido registrar la remesa");
        }
      } else {
        console.log(
...
function registrar_remesa_persona_fecha(
  nid_matricula,
  v_concepto,
  lote,
  fecha_desde,
  ...
  return new Promise((resolve, reject) => {
    async_registrar_remsa_persona_fecha(
      nid_matricula,
      v_concepto,
      lote,
      fecha_desde,
...
async function async_obtener_precio_matricula(nid_matricula, resolve, reject) {
  let v_matricula = await gestor_matricula.obtener_matricula(nid_matricula);
  let nid_persona = v_matricula["nid_persona"];
...
function obtener_precio_matricula(nid_matricula) {
  return new Promise((resolve, reject) => {
    async_obtener_precio_matricula(nid_matricula, resolve, reject);
  });
}

async function async_obtener_precio_matricula_fecha(
  nid_matricula,
  fecha_desde,
  fecha_hasta,
  resolve,
...
function obtener_precio_matricula_fecha(
  nid_matricula,
  fecha_desde,
  fecha_hasta
) {
  return new Promise((resolve, reject) => {
    async_obtener_precio_matricula_fecha(
      nid_matricula,
      fecha_desde,
      fecha_hasta,
      resolve,
...
async function async_obtener_concepto(nid_remesa, resolve, reject) {
  try {
    ...
    console.log("remesa.js - async_obtener_concepto -> " + error);
    reject("Error al obtener el concepto de la remesa");
  }
}

function obtener_concepto(nid_remesa) {
  return new Promise((resolve, reject) => {
    async_obtener_concepto(nid_remesa, resolve, reject);
  });
}
...
async function async_aprobar_remesas(lote, anotaciones, resolve, reject) {
  try {
...
async function aprobar_remesas(lote, anotaciones) {
  return new Promise((resolve, reject) => {
    async_aprobar_remesas(lote, anotaciones, resolve, reject);
  });
}

async function async_actualizar_estado(
  nid_remesa,
  estado,
  anotaciones,
  resolve,
  ...
  try {
    await actualizar_estado(nid_remesa, estado, anotaciones);
    resolve();
  } catch (error) {
    console.log("remesa.js - async_actualizar_estado -> " + error);
    reject("Error al actualizar el estado de la remesa");
  }
}

async function rechazar_remesa(nid_remesa, anotaciones) {
  return new Promise((resolve, reject) => {
    async_actualizar_estado(
      nid_remesa,
...
async function async_actualizacion_remesa(
  v_remesa,
  v_linea_remesa,
  v_descuento_remesa,
  resolve,
