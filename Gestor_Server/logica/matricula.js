...
function obtener_alumnos_asignaturas(nid_curso, nid_asignatura)
{
    return new Promise(
        (resolve, reject) =>
        {
            conexion.dbConn.query('select  p.*, ma.nid_matricula, ma.nid nid_matricula_asignatura from ' + constantes.ESQUEMA_BD + '.matricula_asignatura ma, '+ constantes.ESQUEMA_BD +
        ...
        async (resolve, reject) =>
        {
            conexion.dbConn.beginTransaction(
                async () =>
                {
                    try
                    {
                        let matriculas_a_susituir = await obtener_matriculas_activas_profesor(nid_profesor, nid_asignatura);
