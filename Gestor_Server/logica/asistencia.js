            ...
            conexion.dbConn.beginTransaction(
                () =>
                {
                    conexion.dbConn.query(
                        "insert into " + constantes.ESQUEMA_BD + ".asistentes(nid_evento_asistencia, nid_persona) values(" +
                            conexion.dbConn.escape(nid_evento_asistencia) + ', ' + conexion.dbConn.escape(nid_persona) + ')',
    ...
    return new Promise(
        (resolve, reject) =>
        {
            conexion.dbConn.beginTransaction(
                ...
                {
                    conexion.dbConn.query(
                        "insert into " + constantes.ESQUEMA_BD + '.evento_asistencia(descripcion, fecha) values(' +
                           conexion.dbConn.escape(descripcion) + ', sysdate())',
                        (error, results, fields) =>
                        {
                            if(error) {console.log(error); conexion.dbConn.rollback(); reject(error)}
