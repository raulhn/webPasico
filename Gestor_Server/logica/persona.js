...
function registrar_persona(nombre, primer_apellido, segundo_apellido, telefono, fecha_nacimiento, nif, correo_electronico, codigo)
{
    return new Promise(
        async (resolve, reject) =>
        {
            ...
            if (!bExiste_nif && !bExiste_persona)
            {
                conexion.dbConn.beginTransaction(
                    () =>
                    {  
                        conexion.dbConn.query('insert into ' +  constantes.ESQUEMA_BD + '.persona(nombre, primer_apellido, segundo_apellido, telefono, fecha_nacimiento, nif, correo_electronico, codigo) ' +
...
function existe_forma_pago(nid_forma_pago)
{
    return new Promise(
        (resolve, reject) =>
        {
            conexion.dbConn.query('select count(*) num from ' + constantes.ESQUEMA_BD + '.forma_pago where nid = ' + conexion.dbConn.escape(nid_forma_pago),
                (error, results, fields) =>
                {
                    if(error) {console.log(error); resolve(false)}
                    else {resolve(Number(results[0]['num']) > 0) }
                })
        }
    )
}
