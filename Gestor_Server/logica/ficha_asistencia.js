...
function copiar_ficha_asistencia_alumno(nid_ficha_asistencia, nid_nueva_ficha_asistencia)
{
    return new Promise(
        (resolve, reject) =>
        {
...
function copiar_ficha_asistencia(nombre, fecha, nid_profesor, nid_ficha_asistencia)
{
    return new Promise(
        (resolve, reject) =>
        {
    ...
    return new Promise(
        (resolve, reject) =>
        {
            conexion.dbConn.beginTransaction(
                () =>
                {
                    conexion.dbConn.query('insert into ' + constantes.ESQUEMA_BD + '.ficha_asistencia_alumno(nid_ficha_asistencia, nid_alumno) ' +
                                         'values(' + conexion.dbConn.escape(nid_ficha_asistencia) + ', ' + conexion.dbConn.escape(nid_alumno) + ')',
                        (error, results, fields) =>
                        {
                            if(error) {console.log(error); conexion.dbConn.rollback(); reject(error);}
                            else {conexion.dbConn.commit(); resolve()}
    ...
    return new Promise(
        (resolve, reject) =>
        {
            conexion.dbConn.beginTransaction(
                () =>
                {
                    conexion.dbConn.query('delete from ' + constantes.ESQUEMA_BD + '.ficha_asistencia_alumno where nid_ficha_asistencia_alumno = ' +
    ...
    return new Promise(
        (resolve, reject) =>
        {
            conexion.dbConn.query(
                   ...
                   "   and fam.nid_ficha_asistencia = " + conexion.dbConn.escape(nid_ficha_asistencia),
                (error, results, fields) =>
                {
                    if(error) {console.log(error); reject(error)}
                    else {resolve(results)}
...
function actualizar_ficha_asistencia_alumnos(nid_ficha_asistencia_alumno, asistencia, comentario)
{
    return new Promise(
        (resolve, reject) =>
        {
            conexion.dbConn.beginTransaction(
                () =>
                {
                    conexion.dbConn.query('update ' + constantes.ESQUEMA_BD + '.ficha_asistencia_alumno set asistencia = ' + conexion.dbConn.escape(asistencia) +
                            ', comentario = ' + conexion.dbConn.escape(comentario) + ' where nid_ficha_asistencia_alumno = ' + conexion.dbConn.escape(nid_ficha_asistencia_alumno),
                        (error, results, fields) =>
                        {
                            if(error) {console.log(error); conexion.dbConn.rollback(); reject(error);}
                            else {conexion.dbConn.commit(); resolve();}
